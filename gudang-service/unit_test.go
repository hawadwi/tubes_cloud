...
func TestStartSortingSuccess(t *testing.T) {
	...
	if err != nil {
		t.Logf("StartSorting error (expected in unit test): %v", err)
	}

	if pkg.Status == "sorting" {
		t.Logf("Package status updated to sorting")
	}
}
...
func TestCompleteSortingSuccess(t *testing.T) {
	...
	if err != nil {
		t.Logf("CompleteSorting error (expected in unit test): %v", err)
	}

	if pkg.Status == "ready" {
		t.Logf("Package status updated to ready")
	}
}
